...
void Multi_Texture_1()
{
	...
	if (new_Texture_UV.x < 0.5f) new_Color = texture(u_Texture_1, vec2(new_Texture_UV.x * 2.0f, new_Texture_UV.y));
	else new_Color = texture(u_Texture_2, vec2(fract(new_Texture_UV.x * 2.0f), new_Texture_UV.y));

	FragColor = new_Color;
}


void Multi_Texture_2()
{
    // 12. 멀티텍스쳐 다루기 2.
...
void Multi_Texture_5()
{
    ...
    // => (3) x Offset : mod(floor(u_Time  * animate_Speed), 8) / 8.0f;
    ...
    // => (4) y Offset : mod(floor((u_Time + 1.0f) * 0.5f), 2) * 0.5f;
...
void main()
{
    ...
    //Redirected_Texture_1();
    //Redirected_Texture_2();
    //Redirected_Texture_3();
    //Redirected_Texture_4();
    //Redirected_Texture_5();
    //Redirected_Texture_6();
